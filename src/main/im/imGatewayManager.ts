/**
 * IM Gateway Manager
 * Unified manager for DingTalk, Feishu, Telegram, Discord, QQ, WeCom, Weixin via OpenClaw
 */
...
export class IMGatewayManager extends EventEmitter {
  ...
  private setupGatewayEventForwarding(): void {
    // All platforms run via OpenClaw; no direct gateway events to forward
  }
  ...
  reconnectAllDisconnected(): void {
    ...
    // All platforms run via OpenClaw; no direct reconnect needed
  }
   ...
   * All platforms run via OpenClaw; notification targets are not managed locally.
   */
  private persistNotificationTarget(_platform: Platform): void {
    // All platforms run via OpenClaw; notification targets not managed locally
  }
   ...
   * All platforms run via OpenClaw; notification targets are not managed locally.
   */
  private restoreNotificationTarget(_platform: Platform): void {
    // All platforms run via OpenClaw; notification targets not managed locally
  }
  ...
  setConfig(config: Partial<IMGatewayConfig>, options?: { syncGateway?: boolean }): void {
    ...
    // All platforms run via OpenClaw; config changes are synced via OpenClawConfigSync
