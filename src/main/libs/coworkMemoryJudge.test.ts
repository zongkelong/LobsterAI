...
async function judge(
  text: string,
  { isExplicit = false, guardLevel = 'standard' }: { isExplicit?: boolean; guardLevel?: string } = {},
) {
...
test('Chinese question words 吗/呢 trigger question-like rejection', async () => {
  ...
  expect(r1.reason).toBe('question-like');
  expect(r2.reason).toBe('question-like');
});

// ==================== procedural/command text ====================
...
test('assistant style directive "以后请用中文回复" is accepted when explicit', async () => {
  ...
  expect(r.accepted).toBe(true);
  expect(r.reason).toBe('assistant-preference');
});
...
// ==================== guard level threshold differences ====================
...
test('explicit flag lowers acceptance threshold vs implicit', async () => {
  ...
  if (!implicit.accepted) {
    expect(explicit.accepted).toBe(true); // explicit always at least as permissive
  }
});

test('result always contains required fields', async () => {
  const r = await judge('我是一名前端工程师');
  expect('accepted' in r).toBeTruthy();
  expect('score' in r).toBeTruthy();
  expect('reason' in r).toBeTruthy();
  expect('source' in r).toBeTruthy();
  expect(typeof r.score).toBe('number');
  expect(r.score >= 0 && r.score <= 1).toBeTruthy();
  expect(r.source).toBe('rule');
});
...
// ==================== LLM not called (llmEnabled=false) ====================
...
test('source is always "rule" when llmEnabled is false', async () => {
  ...
  for (const text of texts) {
    const r = await judge(text, { guardLevel: 'standard' });
    expect(r.source).toBe('rule');
  }
});
